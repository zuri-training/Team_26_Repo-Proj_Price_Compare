...
class RequestPasswordResetEmail(APIView):
    def post(self, request):
        ...
        if User.objects.filter(email=email).exists():
            ...
            current_site = get_current_site(request).domain
            relative_url = reverse('accounts:reset-password',
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            if not PasswordResetTokenGenerator().check_token(user, token):
                return Response({'error':'Invalid Token, Please request another token'}, status= status.HTTP_401_UNAUTHORIZED)
            ...
            if not PasswordResetTokenGenerator().check_token(user, token):
                return Response({'error': 'Invalid token, Please request for another token'})
